...
export async function fetchNFDAvatar(nfd: NFD) {
  ...
  if (
    data.properties?.userDefined?.avatar &&
    data.properties.userDefined.avatar.includes("http")
  ) {
  ...
  // IPFS-hosted Avatar
  if (
    data.properties?.verified?.avatar &&
    data.properties.verified.avatar.includes("ipfs")
  ) {
